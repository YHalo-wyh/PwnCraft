...
```python
TargetContext:  # pwnbao/core/session/target.py
    target_id: str            # sha256 前 16 位
    original_binary: str      # 只读原件路径（禁止 patchelf）
    working_binary: str       # .pwnbao/work/runtime/ 工作副本（唯一允许 patch 的文件）
    sha256: str
    file_name: str
    project_root: str
...
- `PwnWorkspace` 新增 `target` 节（schema=2，旧 .pwnbao 兼容加载），`set_target()` 发布 `target_changed`。
...
- `.pwnbao/work/original/` 存原件；`runtime/` 存 patch 后副本。
...
- 目录 `pwnbao/gui/shell/`：`activity_bar.py`、`context_sidebar.py`、`editor_area.py`、`inspector_panel.py`、`bottom_panel.py`。
...
- `pwnbao/gui/terminal/frontend.py`：抽象接口 = 现有 TerminalWidget 对外契约（feed/clear/focus/selected_text/insert_command/submit_inserted/set_command_catalog + bytesTyped/commandSubmitted/terminalResized）。
