# pwn宝 v0.11 Internal Migration Plan
...
7. Populate `/` palette primarily from the live Pwndbg command registry; local Tutor aliases only map to namespaced `pwnbao-*` commands.
...
- Add Tutor commands `pwnbao-frame`, `pwnbao-frames`, `pwnbao-ret`, `pwnbao-rbp`, `pwnbao-stackof`, `pwnbao-safe` without overriding GDB/Pwndbg names.
