...
- 新增 `pwnbao/features/heapviz/grid/physical_grid.py`：
...
- 新增 `pwnbao/features/heapviz/editing/transaction.py`：
...
- 新增 `pwnbao/core/lens.py`：Call Convention / Syscall 两类**寄存器角色表**（amd64/i386/aarch64），事实来源 = ABI 常量 + 真实 syscall 表（含 `SYS_x = 号`、i386 栈传参说明、amd64 `R10 而非 RCX` 等）；`render_lens()` 输出中文解释。
