...
- 新增 `.pwnbao` 项目打开/保存：Dashboard 按钮 + `project.save` / `project.open` 面板条目；`PwnWorkspace.merge_saved()` 在保持对象身份与订阅的前提下整体恢复静态事实，运行时数据自动标记 stale。

## QUERY 工具链（Truth Engine 落地）

- 新增 `pwnbao/core/cli_runner.py`：`CliToolService` 连接 CLI 注册表、执行器、解析器与 Workspace。WSL 未配置时只允许复制命令（执行报错），并保证 QUERY 工具永远不触碰 `exploit` 状态（有测试断言）。
- 新增 `pwnbao/core/static_facts.py`：`readelf -sW`（函数/导入/OBJECT）、`objdump -R`（GOT 槽位）、`objdump -d -j .plt`（PLT 桩）三个严格行格式解析器；任意文本不能伪造成事实。
...
- ROP Chain Builder / ret2libc / ORW / SROP 属 Phase 2/3，本轮未实现（`pwnbao/core/rop.py` 的 Chain 模型与寄存器模拟已就绪待接入）。
