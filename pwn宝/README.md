...
- C 函数速查：工具箱右列速查卡（左侧为编码转换/环境体检/命令提示三卡竖排）+ EXP 工具列 `C 函数` tab（Ctrl+K 直达）。内置 256 个 pwn 常用 C 库函数，覆盖 17 个分类：输入输出（printf 系 / scanf 系 / setvbuf / getline / fmemopen…）、字符串（str* 族 / mem* 族 / bzero 等 BSD 老接口）、内存（malloc 系 / sbrk / mmap / mprotect / 对齐分配…）、文件描述符（open / read / write / dup2 / pipe / pread / fcntl / ioctl / sendfile…）、文件系统（stat / realpath / unlink / chmod…）、进程执行（system / exec* / fork / ptrace / prctl / chroot…）、网络套接字（socket 全家桶 / inet_* 地址转换 / 字节序 / select / poll / epoll）、数值转换（atoi / strto* 族）、随机数（rand / getrandom）、错误诊断（errno / perror / strerror / assert）、信号处理（signal / sigaction / kill / alarm）、系统环境（time / getenv / uid 切换 / rlimit）、字符处理（ctype 全家）、排序查找（qsort / bsearch）、非局部跳转（setjmp / longjmp）、数学（abs / sqrt / pow…）、动态链接（dlopen / dlsym），另附 8 个位/逻辑运算符速查表；每个函数按「原型 · 头文件 · 参数逐项说明 · 返回值 · pwn 笔记」展示，支持搜索、分类过滤与命中计数；数据在 `pwnbao/data/clib_catalog.json`，与代码块目录同一加载/校验模式（`CFunctionCatalog`，经桥 `clib_catalog` 提供）。
