...
- C 函数速查：工具箱右列速查卡（左侧为编码转换/环境体检/命令提示三卡竖排）+ EXP 工具列 `C 函数` tab（Ctrl+K 直达）。内置 79 个 pwn 常用 C 库函数，覆盖 11 个分类：输入输出（printf 系 / scanf 系 / fread / gets / fflush…）、字符串（str* 族 / mem* 族）、内存（malloc 系 / malloc_usable_size / mmap / mprotect / munmap）、文件描述符（open / read / write / dup2 / pipe）、进程执行（system / exec* / fork / waitpid / exit / popen）、网络套接字（socket / connect / bind / listen / accept / send / recv）、数值转换（atoi / strto* 族）、随机数（rand / getrandom）、错误诊断（errno / perror / strerror）、信号处理（signal）、系统环境（time / getenv），另附 8 个位/逻辑运算符速查表；每个函数按「原型 · 头文件 · 参数逐项说明 · 返回值 · pwn 笔记」展示，支持搜索、分类过滤与命中计数；数据在 `pwnbao/data/clib_catalog.json`，与代码块目录同一加载/校验模式（`CFunctionCatalog`，经桥 `clib_catalog` 提供）。
