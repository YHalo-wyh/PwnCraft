...
class PwndbgForkV013Tests(unittest.TestCase):
    ...
    def test_mogai_forces_colors_without_touching_official_theme(self) -> None:
        source = (FORK / "pwndbg/pwnbao/theme.py").read_text(encoding="utf-8")
    ...
    def test_gui_has_no_tutor_renderer_or_command_catalog(self) -> None:
        debugger = ROOT / "pwnbao" / "gui" / "debugger"
    ...
    def test_localization_and_tutor_use_native_pwndbg_apis(self) -> None:
        context_zh = (FORK / "pwndbg/pwnbao/localization/context_zh_CN.py").read_text(encoding="utf-8")
        native = (FORK / "pwndbg/pwnbao/tutor/native.py").read_text(encoding="utf-8")
        context = (FORK / "pwndbg/pwnbao/tutor/context.py").read_text(encoding="utf-8")
    ...
    def test_slash_projects_live_registry_and_real_argparse(self) -> None:
        registry = (FORK / "pwndbg/pwnbao/slash/registry.py").read_text(encoding="utf-8")
        command = (FORK / "pwndbg/pwnbao/slash/command.py").read_text(encoding="utf-8")
    ...
    def test_stop_callback_is_lightweight_and_snapshot_is_explicit(self) -> None:
        source = (FORK / "pwndbg/pwnbao/bridge/runtime.py").read_text(encoding="utf-8")
    ...
    def test_upstream_hooks_are_narrow(self) -> None:
        ...
        self.assertIn("pwndbg.pwnbao.install()", commands)
