...
from pwnbao.features.heapviz import GlibcHeapEngine, analyze_heap_source, build_allocator_config
from pwnbao.features.heapviz.constraints import ConstraintEngine, EditRequest, ValidationStatus, protect_ptr
...
from pwnbao.features.heapviz.corrections import (
    CorrectionEngine,
    CorrectionHistory,
    HelperContractPatch,
    LayoutPatch,
...
from pwnbao.features.heapviz.models import BinState, ChunkField, ChunkState, HeapSnapshot
from pwnbao.features.heapviz.presentation import (
    CanvasLayoutModel,
    CanvasObjectLayout,
...
# pwnbao: op=alloc index=idx size=size data=data
...
def test_visual_partial_one_byte_cross_write_span() -> None:
    from pwnbao.features.heapviz.events import OverwriteEdge
...
def test_helper_variant_benchmark_120_contracts() -> None:
    ...
    for index in range(30):
        for kind, parameters, annotation, call in (
            ...
            blocks.append(f"# pwnbao: op={kind} {annotation}\ndef {name}({parameters}):\n    io.sendline(b'1')")
            calls.append(call)
    source = "\n\n".join(blocks) + "\n\n" + "\n".join(calls)
    resolution = HelperContractResolver().resolve(source)
