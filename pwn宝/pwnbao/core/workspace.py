...
class PwnWorkspace:
    ...
    def merge_saved(self, payload: Mapping[str, object]) -> None:
        """Merge a saved .pwnbao payload into this live workspace instance.
    ...
    def save(self, path: str | Path) -> Path:
        target = Path(path)
        if target.suffix.casefold() != ".pwnbao":
            target = target.with_suffix(".pwnbao")
    ...
    def load(cls, path: str | Path, *, mark_runtime_stale: bool = True) -> "PwnWorkspace":
        ...
        if not isinstance(payload, dict):
            raise ValueError(".pwnbao 文件根节点必须是对象")
