...
from pwndbg.pwnbao.bridge.protocol import emit
...
class RuntimeBridge:
    ...
    def install(self) -> None:
        ...
        class SnapshotCommand(gdb.Command):
            def __init__(self):
                super().__init__("pwnbao-snapshot", gdb.COMMAND_USER)
        ...
        try:
            from pwndbg.pwnbao.slash.registry import commands, option_hints
            command_catalog = [
                {
                    "name": item.name,
    ...
    def _on_before_prompt(self) -> None:
        ...
        try:
            from pwndbg.pwnbao.theme import ensure_classic_colors
...
def install_if_enabled() -> RuntimeBridge | None:
    global _runtime
    if os.environ.get("PWNBAO_BRIDGE", "").casefold() not in {"1", "true", "yes"}:
        return None
    if _runtime is None:
