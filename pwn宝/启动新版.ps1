...
if (Test-Path (Join-Path $here 'pwnbao-electron/package.json')) {
  $root = $here
} elseif (Test-Path (Join-Path $here 'pwn宝/pwnbao-electron/package.json')) {
  $root = Join-Path $here 'pwn宝'
} else {
  throw "找不到 pwnbao-electron/package.json，当前目录：$here"
}
...
$electronDir = Join-Path $root 'pwnbao-electron'
