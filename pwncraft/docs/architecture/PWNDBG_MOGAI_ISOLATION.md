...
```text
官方 Pwndbg（PwnCraft只读、不修改）
  command: command -v pwndbg
  config/cache/source: 用户原有位置

PwnCraft魔改版
  command: ~/.local/bin/pwndbg-mogai
  root:    ~/.local/share/pwncraft/pwndbg-mogai/2026.07.29-pwndbg-mogai.16
  source:  <root>/source
  runtime: <root>/runtime
  config:  ~/.config/pwncraft/pwndbg-mogai
  cache:   ~/.cache/pwncraft/pwndbg-mogai
  data:    ~/.local/share/pwncraft/pwndbg-mogai/data
```
...
PyInstaller one-file 进程启动系统 `wsl.exe` 前调用 `SetDllDirectoryW(None)`，清除 `_MEI` 私有 DLL 搜索目录，避免 WSL 错误加载打包目录 DLL。入口：`pwncraft/core/external_process.py`。这只修复 Windows 子进程装载环境，不改 WSL 内部状态。
