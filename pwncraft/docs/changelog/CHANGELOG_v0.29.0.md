...
- `pwncraft/electron_bridge.py`：全面扩容。新增 RPC：
...
- `pwncraft/features/heapviz/bridge_session.py`（新增）：无头堆会话。
...
## 三、Electron 端（pwncraft-electron/）
...
- 🔶 PyQt gui/ 保留为参考实现不再启动；`run_pwncraft.py` 入口默认仍指向 PyQt（可后续切换/删除）
