...
UI Theme 大改（按《PwnCraft UI Theme 大改提示词》）：**Dark Workbench**。只改 Theme/Typography/Spacing/Widget Style/交互反馈/视觉层级；Heap Truth、PhysicalMemory、Analyzer、TargetContext、pwndbg-mogai、Runtime Bridge、EXP 语义**零改动**。

## Theme 架构（§32）

- `pwncraft/gui/theme.py` 单文件升级为 **`pwncraft/gui/theme/` 包**：
...
- 旧导入路径 `from pwncraft.gui.theme import Colors, build_stylesheet, FIXED_UI_SCALE` 完全兼容；scale 不变性测试继续通过。
