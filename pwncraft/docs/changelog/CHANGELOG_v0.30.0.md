...
- 真值层身份：`pwncraft.__init__` `APP_NAME = "PwnCraft"`、`APP_VERSION = "v0.30.0"`。
  Python 包名 `pwncraft`、`.pwncraft` 项目目录、localStorage 键名**不变**（代码身份，非品牌）。
...
- `pwncraft/core/session/target.py`：`import_target` 幂等修复（`_refresh_copy`）——
