...
(() => {
  ...
  function bindGlobalDragDrop() {
    ...
    document.addEventListener('drop', (event) => {
      ...
      if (!file) return;
      let path = '';
      if (window.pwncraft.filePathFor) path = window.pwncraft.filePathFor(file);
  ...
  function recentPaths() {
    try { return JSON.parse(localStorage.getItem('pwncraft.recentPaths') || '[]'); }
    catch { return []; }
  }
  function pushRecentPath(path) {
    const list = recentPaths().filter((item) => item !== path);
    list.unshift(path);
    localStorage.setItem('pwncraft.recentPaths', JSON.stringify(list.slice(0, 5)));
  }
  ...
  async function pickAndImport() {
    const path = await window.pwncraft.pickElf();
  ...
  async function importElf(path) {
    ...
    try {
      const result = await window.pwncraft.request('import_target', { path, light: true });
  ...
  async function applyWorkspace(entry, options = {}) {
    ...
    window.PwnPages?.scanVulnPoints(entry);
  ...
  async function startAutoTriage(entry) {
    ...
    state.triageFor = entry.context.working_binary || entry.path;
    try {
      const result = await window.pwncraft.request('auto_triage', {});
  ...
  function initMonaco() {
    ...
    window.require(['vs/editor/editor.main'], () => {
      ...
      monaco.editor.defineTheme('pwncraft-dark', {
  ...
  async function syncExpToBridge() {
    try {
      await window.pwncraft.request('exp_set', { text: getExpText() });
  ...
  async function saveExp() {
    ...
    const path = await window.pwncraft.pickSavePath(defaultName, 'py');
    if (!path) return;
    try {
      await window.pwncraft.request('exp_save', { path, text: getExpText() });
      log(`已保存: ${path}`);
    } catch (error) {
  ...
  async function createTerminal(kind = 'shell') {
    ...
    term.open(host);
    term.onData((data) => {
      if (activeTerminalId !== null) window.pwncraft.terminalInput(activeTerminalId, data);
    });
    const refit = () => {
      try {
        fitAddon.fit();
        const id = Number(host.dataset.termId);
        if (id) window.pwncraft.terminalResize(id, term.cols, term.rows);
      } catch { /* hidden */ }
    };
  ...
  function renderTerminalTabs() {
    ...
    for (const [id, entry] of terminals) {
      ...
      close.addEventListener('click', async (event) => {
        ...
        await window.pwncraft.terminalKill(id);
        removeTerminal(id);
      });
  ...
  async function restartShellTerminal(cwd) {
    ...
    for (const [id, entry] of [...terminals]) {
      if (entry.kind === 'shell') {
        try { await window.pwncraft.terminalKill(id); } catch { /* gone */ }
        removeTerminal(id);
      }
    }
  ...
  async function runInTerminal(command) {
    ...
    await window.pwncraft.terminalInput(shellId, `${command}\r`);
    log(`已写入终端：${command}`);
  }
  ...
  async function sendToDebugTerminal(command) {
    ...
    await window.pwncraft.terminalInput(debugId, `${command}\r`);
  }
  ...
  function attachDbgCompletion(entry) {
    ...
    function acceptCompletion() {
      ...
      if (rest) {
        term.write(rest);
        window.pwncraft.terminalInput(entry.id, rest);
        entry.input = m.n;
      }
      hidePopup();
    }
        ...
        const renderList = (needle) => {
          ...
          list.querySelectorAll('.tcs-row').forEach((row) => {
            row.addEventListener('click', () => {
              ...
              term.write(command);
              window.pwncraft.terminalInput(entry.id, command);
              entry.input = command;
              term.focus();
            });
          });
  ...
  function attachDebugTerminal(id) {
    ...
    term.onData((data) => {
      ...
      window.pwncraft.terminalInput(id, data);
    });
    const refit = () => {
      try {
        fitAddon.fit();
        window.pwncraft.terminalResize(id, term.cols, term.rows);
      } catch { /* hidden */ }
    };
    new ResizeObserver(refit).observe(host);
  ...
  window.pwncraft.onTerminalData(({ id, data }) => {
    const entry = terminals.get(id);
    if (!entry) return;
  ...
  window.pwncraft.onTerminalExit(({ id, exitCode, kind }) => {
  ...
  function bindBridgeEvents() {
    window.pwncraft.onBridgeEvent((payload) => {
  ...
  async function boot() {
    ...
    updateStatusLeft();
    try {
      const pong = await window.pwncraft.request('ping');
