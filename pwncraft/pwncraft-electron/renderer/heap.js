...
(() => {
  ...
  async function applyAllocatorProfile() {
    ...
    try {
      const heapState = await window.pwncraft.request('heap_reprofile', { allocator: request });
  ...
  async function loadTemplates() {
    const result = await window.pwncraft.request('heap_templates');
  ...
  async function loadTemplate(templateId) {
    try {
      ...
      state.scenarioName = '';
      const heapState = await window.pwncraft.request('heap_load', {
  ...
  function autoReplayFromExp() {
    ...
    (async () => {
      try {
        if (!text.trim()) {
          const empty = await window.pwncraft.request('heap_load', {
  ...
  async function executeLiveOp() {
    ...
    try {
      const heapState = await window.pwncraft.request('heap_operation', payload);
...
`);
  ...
  async function canonicalExpLine(kind, payload, info) {
    ...
    try {
      const result = await window.pwncraft.request('heap_canonical_python', {
        kind,
        chunk: label,
  ...
  function renderLearnSummary() {
    ...
    host.querySelectorAll('.mapping-del').forEach((button) => {
      button.addEventListener('click', async () => {
        try {
          const result = await window.pwncraft.request('heap_mapping', {
    ...
    host.querySelectorAll('.rec-item').forEach((item) => {
      ...
      if (confirm) {
        confirm.addEventListener('click', async () => {
          ...
          try {
            const result = await window.pwncraft.request('heap_label_candidate', {
  ...
  async function locateChunk(chunkId) {
    try {
      const result = await window.pwncraft.request('heap_chunk_history', {
  ...
  async function locateField(address) {
    try {
      const result = await window.pwncraft.request('heap_field_provenance', {
  ...
  function renderPending() {
    ...
    $('#pending-simulate').addEventListener('click', async () => {
      try {
        const result = await window.pwncraft.request('heap_apply_pending', {
  ...
  async function solvePendingFromPatch(step, patch) {
    try {
      const result = await window.pwncraft.request('heap_reverse_edit', {
  ...
  async function commitResize(chunk, edge, delta) {
    ...
    try {
      const result = await window.pwncraft.request('heap_structural_edit', {
        step: state.current,
  ...
  function showCorrectionDialog(chunk, cell = null) {
    ...
    `, async () => {
      ...
      try {
        const result = await window.pwncraft.request('heap_correct', {
          step: corrStep,
          intent: canvasIntent,
          patch,
          context: {
  ...
  async function undoCorrection() {
    try {
      const result = await window.pwncraft.request('heap_undo_correction');
      if (result && result.ok === false) {
  ...
  async function exportTrainingCase() {
    try {
      ...
      const path = await window.pwncraft.pickSavePath(defaultName, 'json');
      if (!path) return;
      const result = await window.pwncraft.request('heap_export_case', {
        path,
        target: { arch },
        challenge_family: '',
      });
  ...
  async function importAgentReview() {
    try {
      const path = await window.pwncraft.pickOpenPath('json');
      if (!path) return;
      const result = await window.pwncraft.request('heap_import_review', { path });
  ...
  async function applyAgentReview(reviewId, humanConfirmed) {
    try {
      const result = await window.pwncraft.request('heap_apply_review', {
  ...
  async function refreshReviewLayer() {
    const fresh = await window.pwncraft.request('heap_state');
  ...
  async function saveScene() {
    try {
      ...
      const path = await window.pwncraft.pickSavePath(defaultName, 'json');
      if (!path) return;
      await window.pwncraft.request('heap_save', { path });
      log(`堆场景已保存：${path}`);
    } catch (error) {
  ...
  async function openScene() {
    try {
      const path = await window.pwncraft.pickOpenPath('json');
      if (!path) return;
      const heapState = await window.pwncraft.request('heap_open', { path });
  ...
  async function renderIoFilePage() {
    ...
    try {
      const layout = await window.pwncraft.request('iofile_layout', {
      ...
      const analysis = expSource
        ? await window.pwncraft.request('iofile_analyze', { source: expSource })
        : { evidence: [] };
    ...
    : '<div class="hint-dim">exp.py 中暂无 FILE 相关写入证据。</div>'}
      ...
      host.querySelectorAll('.iofile-value').forEach((input) => {
        input.addEventListener('change', async () => {
          ...
          try {
            const result = await window.pwncraft.request('iofile_validate', {
              version: effectiveIoFileVersion(),
