 ...
 * truth bridge (stdio JSON-RPC to `pwncraft.electron_bridge`), and terminal
...
const PYTHON = process.env.PWNCRAFT_PYTHON || 'python';
...
if (SHOT || SMOKE) {
  app.setPath('userData', path.join(app.getPath('temp'), `pwncraft-check-profile-${process.pid}`));
}
...
function startBridge() {
  const child = spawn(PYTHON, ['-u', '-m', 'pwncraft.electron_bridge'], {
    cwd: PROJECT_ROOT,
...
async function runShots() {
  ...
  // PWNCRAFT_SHOT_IMPORT=<绝对路径>：用真实 ELF 复现导入链路（默认仍造最小假 ELF）。
  const importPath = process.env.PWNCRAFT_SHOT_IMPORT || '';
  let elfDir;
  let targetPath;
  if (importPath) {
    ...
    elfDir = path.join(app.getPath('temp'), `pwncraft-shot-target-${Date.now()}`);
  ...
  await run(`window.__pwncraftDebug.importElf(${JSON.stringify(targetPath)})`);
  await sleep(9000); // WSL static tools + terminal restart
  await shot('02-binary.png');

  await run('window.__pwncraftDebug.switchPage("heap")');
  ...
  await run('window.__pwncraftDebug.switchPage("heap")');
  await run('window.__pwncraftDebug.heapAdvance && window.__pwncraftDebug.heapAdvance(6)');
  await sleep(2500);
  await shot('03-heap.png');

  await run('window.__pwncraftDebug.switchPage("rop")');
  await sleep(1500);
  await shot('04-rop.png');

  await run('window.__pwncraftDebug.switchPage("debug")');
  await sleep(1500);
  await shot('05-debug.png');

  await run('window.__pwncraftDebug.switchPage("welcome"); window.__pwncraftDebug.openPalette()');
  await sleep(1200);
