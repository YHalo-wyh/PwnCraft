...
class PwndbgManager:
    ...
    def __init__(self, wsl_exe: str = "wsl.exe", download_root: str | Path | None = None):
        ...
        default_root = Path(local) / "pwncraft" / "downloads" if local else Path.home() / ".pwncraft" / "downloads"
        self.download_root = Path(download_root) if download_root else default_root
        self.mogai_root = f"$HOME/.local/share/pwncraft/pwndbg-mogai/{PWNDBG_MOGAI_VERSION}"
        ...
        self.config_root = "$HOME/.config/pwncraft/pwndbg-mogai"
        self.cache_root = "$HOME/.cache/pwncraft/pwndbg-mogai"
        self.data_root = "$HOME/.local/share/pwncraft/pwndbg-mogai/data"
        self._legacy_runtime = f"$HOME/.local/share/pwncraft/pwndbg/{PWNDBG_VERSION}"
    ...
    def ensure_installed(self, progress: Callable[[str], None] | None = None) -> str:
        ...
        if legacy_available:
            emit("发现 PwnCraft旧版私有 runtime，离线复制到 pwndbg-mogai；不读取官方 pwndbg")
        else:
        ...
        # old PwnCraft portable runtime may be copied as an offline migration,
        ...
        script = (
            ...
            "#!/bin/sh\n"
            f'exec sh "$HOME/.local/share/pwncraft/pwndbg-mogai/{PWNDBG_MOGAI_VERSION}/source/launcher.sh" '
            "--quiet -nx \"$@\"\n"
            "PWNDBG_MOGAI_EOF\n"
    ...
    def launch_spec(self, elf_path: str | Path) -> PwndbgLaunchSpec:
        ...
        startup = ""
        command = (
            f'PWNCRAFT_BRIDGE=1 exec "{self.command_path}" -nx '
    ...
    def _download(
        ...
        request = urllib.request.Request(url, headers={"User-Agent": "pwncraft-pwndbg-installer/1"})
        downloaded = 0
        next_report = 0
