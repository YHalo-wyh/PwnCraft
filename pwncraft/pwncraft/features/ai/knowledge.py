...
def default_knowledge_path() -> Path:
    ...
    if base:
        return Path(base) / "pwncraft" / "ai" / "knowledge.sqlite3"
    return Path.home() / ".local" / "share" / "pwncraft" / "ai" / "knowledge.sqlite3"
...
class AIKnowledgeStore:
    ...
    def __init__(self, path: str | os.PathLike[str] | None = None):
        ...
        try:
            ...
            self.path = Path(tempfile.gettempdir()) / f"pwncraft-ai-knowledge-{os.getpid()}-{uuid.uuid4().hex}.sqlite3"
            self.path.parent.mkdir(parents=True, exist_ok=True)
            self._initialize()
