...
class _PromptSession:
    def __init__(self, target: Path, *, relay: bool):
        ...
        environment.update(TERM="xterm-256color", COLORTERM="truecolor", PWNCRAFT_BRIDGE="0")
        ...
        if relay:
            command = [
                "python3", str(ROOT / "pwncraft/core/terminal/pty_relay.py"),
...
def _compile_probe(directory: Path) -> Path:
    source = directory / "pwncraft_perf.c"
    target = directory / "pwncraft_perf"
    source.write_text(
        "#include <stdlib.h>\n"
...
def run() -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="pwncraft-pwndbg-perf-") as temporary:
