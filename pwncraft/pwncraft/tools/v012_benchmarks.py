...
from pwncraft.features.heapviz import build_allocator_config
from pwncraft.features.heapviz.constraints import ConstraintEngine, EditRequest, ValidationStatus
...
from pwncraft.features.heapviz.corrections import CorrectionEngine, HelperContractPatch, ObservedMemoryPatch
from pwncraft.features.heapviz.corrections.rebase import rebase_fingerprint
from pwncraft.features.heapviz.semantics import CanonicalOperationKind
...
def helper_variant_benchmark() -> dict[str, object]:
    ...
    for index in range(30):
        ...
        for kind, parameters, roles, call in variants:
            ...
            blocks.append(f"# pwncraft: op={kind} {roles}\ndef {name}({parameters}):\n    io.sendline(b'1')")
            calls.append(call)
    source = "\n\n".join(blocks) + "\n\n" + "\n".join(calls)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="PwnCraft v0.12 deterministic offline benchmarks")
