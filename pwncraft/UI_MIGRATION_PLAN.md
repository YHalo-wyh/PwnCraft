...
- 前端：`pwncraft-electron/`（Electron + Monaco + xterm.js + node-pty）。
- 真值层：`pwncraft/electron_bridge.py` 通过 stdio JSON-RPC 调用 `pwncraft.core` 与 `pwncraft.features`。
...
- 第三方依赖以 `pwncraft-electron/package.json` 和 `requirements.txt` 为准。
