...
PwnCraft是一个 Windows 优先的 CTF Pwn EXP 工作台，**当前主程序为 Electron Workbench**（`pwncraft-electron/`）：
Electron 壳 + Monaco（EXP 编辑器）+ xterm.js/node-pty（多实例终端）+ Python 真值桥
（`pwncraft.electron_bridge`，30+ stdio JSON-RPC）。Electron 端永远只是表面，所有 Pwn 真值
...
```text
pwncraft/            源码主包（features / core / tools + electron_bridge）
pwncraft-electron/   Electron Workbench（唯一主程序：壳 / 渲染层 / node-pty 终端）
tests/             自动化测试（pytest）
third_party/       pwndbg-mogai 独立源码 Fork（源码的一部分）
datasets/          基准数据集
pwncraft/core/session/  TargetContext 单一 Target 真值
docs/
...
- **Dashboard**：目标优先入口（检查程序 / 找 Gadget / 构造 ROP / 绕 Seccomp …）、项目打开/保存（`.pwncraft`）、变量面板与实时摘要。
...
- AI 知识默认保存到 `%LOCALAPPDATA%\pwncraft\ai\knowledge.sqlite3`，仅持久化候选相关源码片段，不保存整份 EXP；API token 只存在当前进程。非 loopback Base URL 会明确提示完整 EXP 将被发送。
...
- PwnCraft仅调用独立 `pwndbg-mogai`：source/runtime/config/cache/data 全部位于 PwnCraft命名空间；官方 `pwndbg`、`.gdbinit`、插件和更新方式完全不改。portable runtime 仍固定为 `2026.07.29` 并校验官方 SHA256。

## 运行

    cd C:\Users\WYH\Desktop\pwncraft\pwncraft\pwncraft-electron
    npm install        # 首次
    npm start

前置：Node.js（npmmirror 镜像）与系统 Python 3（真值桥 `pwncraft.electron_bridge`，标准库即可；
...
- 请直接使用上述模型 ID，不再使用 `pwncraft-qwen-coder` 别名，避免 LM Studio 的模型元数据解析异常。
...
仓库基准清单位于 `datasets/heap_ai/manifest.json`。本地 EXP 只按绝对路径和 SHA256 读取，不复制进仓库；how2heap 来源锁定 MIT commit。批处理支持按 split、case、SHA256 缓存和断点恢复：
    ...
    python -m pwncraft.tools.heap_ai_corpus datasets/heap_ai/manifest.json `
...
本地 PDF WriteUp 可先生成只含哈希、来源页、修复后 Python 片段和派生 case 的清单；原 PDF 不会复制到项目：
    ...
    python -m pwncraft.tools.heap_ai_writeups F:\PWN\PWN\99PWN\00_WriteUp `
...
## 模块结构
    ...
    pwncraft/core/workspace          统一 PwnWorkspace、事件总线、TypedAddress、.pwncraft 持久化
    pwncraft/core/tool_actions       QUERY/DERIVE/BUILD/VERIFY/RUNTIME Action 类型系统
    pwncraft/core/cli_registry       CLI 工具注册表（中文参数说明）与命令构造
    pwncraft/core/cli_runner         CliToolService：执行 → 严格解析 → Workspace 入库
    pwncraft/core/static_facts       readelf/objdump 输出的结构化符号解析器
    pwncraft/core/workbench          BinaryInspector、GadgetExplorer、SyscallPlanner、编码与命令面板
    pwncraft/electron_bridge         Electron 真值桥（stdio JSON-RPC，30+ 方法）
    pwncraft/features/blocks        代码块、占位符和代码块渲染入口
    pwncraft/features/conversion    进制转换、p32/p64、u32/u64、signed/unsigned 结果
    pwncraft/features/fmtstr        格式化字符串 payload 生成器
    pwncraft/features/commands      WSL 命令提示和 GDB/Pwndbg 指令模板
    pwncraft/features/ai            LM Studio Provider、Heap Rule Catalog、严格校验、语料与 SQLite 反馈库
    pwncraft/features/heapviz       堆操作 IR、glibc 配置、模拟重放和代码生成
    pwncraft/features/heapviz/memory     稀疏 PhysicalMemory、地址空间、provenance
    pwncraft/features/heapviz/payload    PayloadIR 与安全 AST payload evaluator
    pwncraft/features/heapviz/events     alloc/free/read/write/overwrite 因果事件
    pwncraft/features/heapviz/views      从 PhysicalMemory 读取的 typed chunk views
    pwncraft/features/heapviz/benchmark  ground-truth semantic benchmark 与指标
    pwncraft/features/heapviz/bridge_session  Electron 无头堆会话（校正→回放→规则学习）
    pwncraft/core/elf_runtime         同目录 runtime 发现、patchelf 验证与回滚
    pwncraft/core/pwndbg_manager      固定 portable release 下载、SHA256 验证与 WSL 启动
