...
MOGAI_BASE="${PWNDBG_MOGAI_HOME:-$HOME/.local/share/pwncraft/pwndbg-mogai}"
PORTABLE_ROOT="${PWNDBG_PWNCRAFT_PORTABLE:-$MOGAI_BASE/$MOGAI_VERSION/runtime}"
...
export XDG_CONFIG_HOME="${PWNDBG_MOGAI_CONFIG_HOME:-$HOME/.config/pwncraft/pwndbg-mogai}"
export XDG_CACHE_HOME="${PWNDBG_MOGAI_CACHE_HOME:-$HOME/.cache/pwncraft/pwndbg-mogai}"
export XDG_DATA_HOME="${PWNDBG_MOGAI_DATA_HOME:-$HOME/.local/share/pwncraft/pwndbg-mogai/data}"
export GDBHISTFILE="${PWNDBG_MOGAI_GDBHISTFILE:-$XDG_DATA_HOME/gdb_history}"
mkdir -p "$XDG_CONFIG_HOME" "$XDG_CACHE_HOME" "$XDG_DATA_HOME"
export PWNDBG_PWNCRAFT_LD="$PORTABLE_ROOT/lib/ld-linux-x86-64.so.2"
export PWNDBG_PWNCRAFT_PYTHON="$PORTABLE_ROOT/exe/python3"
