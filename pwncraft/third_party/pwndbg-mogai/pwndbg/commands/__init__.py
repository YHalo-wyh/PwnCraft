...
def load_commands() -> None:
    ...
    # PwnCraft keeps its additions behind one narrow upstream hook.  The fork
    # modules consume the command registry only after upstream finished
    # registering every command, so slash/help are always a projection of the
    # live registry rather than a second catalog maintained by the GUI.
    import pwndbg.pwncraft

    pwndbg.pwncraft.install()
