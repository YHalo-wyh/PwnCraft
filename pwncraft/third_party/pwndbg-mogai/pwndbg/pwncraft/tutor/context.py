...
def context_pwncraft(target=None, width=None, height=None, with_banner=True) -> list[str]:
    ...
    from pwndbg.pwncraft.tutor.frame_state import inspect_frame
...
def install() -> None:
    ...
    context.context_sections["p"] = context_pwncraft
    sections = str(context.config_context_sections.value).split()
    if "pwncraft" not in sections:
        # Keep the upstream sections and append one compact native section.
        context.config_context_sections.value = " ".join((*sections, "pwncraft"))
    _installed = True
