...
from pwndbg.pwncraft.slash.registry import NativeCommand, by_name, commands, option_hints, search
from pwndbg.pwncraft.localization.commands_zh_CN import search_keywords
...
def install() -> None:
    ...
    if os.environ.get("PWNCRAFT_SLASH_PROXY", "").casefold() in {"1", "true", "yes"}:
        payload = [
            {
                "name": item.name,
